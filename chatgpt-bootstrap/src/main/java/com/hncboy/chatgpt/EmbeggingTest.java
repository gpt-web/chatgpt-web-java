...
public class EmbeggingTest {
	...
	public static void main(String[] args) {
		...
		int subSize = 3;
		List<List<Map>> subLists = IntStream.range(0, (dataList.size() + subSize - 1) / subSize)
				.mapToObj(i -> dataList.subList(i * subSize, Math.min((i + 1) * subSize, dataList.size())))
		...
		try {
			for (List<Map> subList : subLists) {
				List<String> input = new ArrayList<>();
				// 遍历打印数据
				for (Map rowData : subList) {
				...
				for (int i = 0; i < resultList.size(); i++) {
					...
					// 将向量保存至milvus
					InsertParam insertParam = InsertParam.newBuilder().withCollectionName("kownledge")
							.withFields(fields).build();
					R<MutationResult> resultData = milvusServiceClient.insert(insertParam);
					if (resultData.getStatus() == 0) {
						System.out.println("第" + n + "行写入成功");
					} else {
						System.out.println("第" + n + "行写入失败");
