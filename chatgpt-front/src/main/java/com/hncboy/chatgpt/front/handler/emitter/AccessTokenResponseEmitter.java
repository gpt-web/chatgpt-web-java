...
/**
 * @author hncboy
 * @date 2023-3-24 AccessToken 响应处理
 */
@Component
public class AccessTokenResponseEmitter implements ResponseEmitter {
    ...
    public void requestToResponseEmitter(ChatProcessRequest chatProcessRequest, ResponseBodyEmitter emitter) {
        ...
        // 如果是第一条消息，添加系统提示
        if (chatMessageDO.getContextCount() == 1 && StringUtils.isNotBlank(chatProcessRequest.getSystemMessage())) {
        	chatMessageDO.setContent(chatProcessRequest.getSystemMessage() + "我的第一个问题是：" + chatMessageDO.getContent());
        }
...
//                .addListener(new ConsoleStreamListener())
	...
	private ConversationRequest buildConversationRequest(ChatMessageDO chatMessageDO) {
		...
		ConversationRequest.Content content = ConversationRequest.Content.builder()
				.parts(Collections.singletonList(chatMessageDO.getContent())).build();

		// 构建 Message
		ConversationRequest.Message message = ConversationRequest.Message.builder().id(chatMessageDO.getMessageId())
				.role(Message.Role.USER.getName()).content(content).build();

		// 构建 ConversationRequest
		String model = chatConfig.getOpenaiApiModel();

		return ConversationRequest.builder().messages(Collections.singletonList(message))
				.action(ConversationRequest.MessageActionTypeEnum.NEXT).model(ConversationModelEnum.NAME_MAP.get(model))
				// 父级消息 id 不能为空，不然会报错，因此第一条消息也需要随机生成一个
				.parentMessageId(StrUtil.isBlank(chatMessageDO.getParentMessageId()) ? UUID.randomUUID().toString()
						: chatMessageDO.getParentMessageId())
				.conversationId(chatMessageDO.getConversationId()).build();
	}
}
